...
const News = () => {
  ...
  return (
    <div className='py-10 border-2 rounded-md shadow-md'>
      <h2 className='text-3xl font-bold mb-6 ms-12' c>News</h2>
      <Swiper
        slidesPerView={1}
        spaceBetween={30}
        navigation={true}
      ...
      >
        ...
        news.map((Item,index)=>(
              <SwiperSlide key={index}>
                <div className='flex flex-col sm:flex-row sm:justify-between items-center gap-12 mx-12'>
                    <div className='py-4 '>
                     <Link to={'/'}>
                     <h3 className='text-lg font-medium hover:text-blue-500 mb-4'>{Item.title}</h3>
                     </Link>
                     <div className='w-10 h-[4px] bg-yellow-400 mb-5'></div>
